...
export function testEmailTemplate(settings, recipient) {
  const sentAt = new Date();
  const text = [
    "Nimbus Direct email delivery is working.",
    "",
    `This test was sent to ${recipient}.`,
    `SMTP endpoint: ${settings.host}:${settings.port}`,
    `Encryption: ${settings.security === "tls" ? "TLS" : "STARTTLS"}`,
...
<html lang="en"><body style="margin:0;background:#f4f6fb;color:#1d2740;font-family:Arial,sans-serif">
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;overflow:hidden;border:1px solid #e0e4ef;border-radius:18px;background:#ffffff">
        ...
        <tr><td style="padding:34px 28px">
          <div style="display:inline-block;padding:7px 11px;border-radius:999px;background:#eaf8f2;color:#16865f;font-size:12px;font-weight:700">Delivery verified</div>
          <h1 style="margin:18px 0 10px;font-size:26px;line-height:1.25">Your email channel is ready.</h1>
          <p style="margin:0 0 22px;color:#667087;font-size:15px;line-height:1.65">Nimbus Direct successfully authenticated with your SMTP server and delivered this message.</p>
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;border-spacing:0 8px">
            <tr><td style="color:#8a93a8;font-size:12px">Recipient</td><td align="right" style="font-size:13px;font-weight:700">${escapeHtml(recipient)}</td></tr>
            <tr><td style="color:#8a93a8;font-size:12px">SMTP endpoint</td><td align="right" style="font-size:13px;font-weight:700">${escapeHtml(settings.host)}:${Number(settings.port)}</td></tr>
            <tr><td style="color:#8a93a8;font-size:12px">Encryption</td><td align="right" style="font-size:13px;font-weight:700">${settings.security === "tls" ? "TLS" : "STARTTLS"}</td></tr>
          </table>
        </td></tr>
  ...
  return {
    subject: "Nimbus Direct email delivery test",
    text,
    html,
  };
}

export function securityEmailTemplate({ displayName, title, message, ipAddress = null, occurredAt = new Date() }) {
  const name = String(displayName || "there").trim();
  const safeTitle = oneLine(title, "Security email title", 160);
  const safeMessage = oneLine(message, "Security email message", 500);
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 14px;background:#f4f6fb">
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;overflow:hidden;border:1px solid #e0e4ef;border-radius:18px;background:#ffffff">
        <tr><td style="padding:24px 28px;background:#11182a;color:#ffffff;font-size:20px;font-weight:700">nimbus <span style="color:#7580ff">direct</span></td></tr>
        <tr><td style="padding:34px 28px">
          <div style="display:inline-block;padding:7px 11px;border-radius:999px;background:#eef0ff;color:#5662dc;font-size:12px;font-weight:700">Account security</div>
          <h1 style="margin:18px 0 10px;font-size:26px;line-height:1.25">${escapeHtml(safeTitle)}</h1>
          <p style="margin:0 0 22px;color:#667087;font-size:15px;line-height:1.65">Hello ${escapeHtml(name)}, ${escapeHtml(safeMessage)}</p>
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;border-spacing:0 8px">
            <tr><td style="color:#8a93a8;font-size:12px">Time</td><td align="right" style="font-size:13px;font-weight:700">${escapeHtml(time.toISOString())}</td></tr>
            ${ipAddress ? `<tr><td style="color:#8a93a8;font-size:12px">IP address</td><td align="right" style="font-size:13px;font-weight:700">${escapeHtml(ipAddress)}</td></tr>` : ""}
          </table>
          <div style="margin-top:24px;padding:15px 16px;border-radius:12px;background:#fff4e8;color:#8a531d;font-size:13px;line-height:1.55">If you did not make this change, contact your infrastructure provider immediately and change your Nimbus Direct password.</div>
        </td></tr>
        <tr><td style="padding:18px 28px;border-top:1px solid #edf0f5;color:#929aad;font-size:11px">Automatic security notice from Nimbus Direct</td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`;
...
function accountActionEmailTemplate({
  ...
  const text = [
    ...
    "",
    `${actionLabel}: ${safeUrl}`,
    `This link expires at ${expiry.toISOString()} and can be used once.`,
    "",
    ignoreMessage,
  ].join("\n");
  const html = `<!doctype html>
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;overflow:hidden;border:1px solid #e0e4ef;border-radius:18px;background:#ffffff">
        ...
        <tr><td style="padding:34px 28px">
          ...
          <p style="margin:20px 0 0;color:#8a93a8;font-size:12px;line-height:1.55">This private link expires at ${escapeHtml(expiry.toISOString())} and can be used once.</p>
        ...
        <tr><td style="padding:18px 28px;border-top:1px solid #edf0f5;color:#929aad;font-size:11px">Automatic account message from Nimbus Direct</td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`;
...
export function invitationEmailTemplate({ displayName, customerName, actionUrl, expiresAt }) {
  return accountActionEmailTemplate({
    displayName,
    subject: "You are invited to Nimbus Direct",
    badge: "Account invitation",
    title: "Your infrastructure account is ready.",
    ...
    actionLabel: "Create my password",
    actionUrl,
    expiresAt,
...
export function passwordResetEmailTemplate({ displayName, actionUrl, expiresAt }) {
  return accountActionEmailTemplate({
    displayName,
    subject: "Reset your Nimbus Direct password",
    badge: "Password recovery",
    title: "Reset your password.",
    message: "A password reset was requested for your Nimbus Direct account.",
    actionLabel: "Reset my password",
    actionUrl,
    expiresAt,
    ignoreMessage: "If you did not request this reset, no action is required and your current password remains valid.",
  });
}
...
export function createEmailService({
  ...
  async function sendTest(recipient, createdBy) {
    ...
    const content = testEmailTemplate(settings, to);
